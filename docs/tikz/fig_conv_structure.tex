...
\begin{tikzpicture}[node distance=8mm and 12mm]
  ...
  \node[lbl, text=ethgrey] at ($(volt.south)+(2.4,-0.65)$)
    {\footnotesize \textbf{GridForming}: droop angle source;\ \ \textbf{GridSupporting}: PLL angle source; each block is a pluggable strategy (\textbf{GridFollowing} is a separate current-injecting chain)};
\end{tikzpicture}
\end{document}
