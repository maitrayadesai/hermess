...
# PSID reference for the pscad/test23 case: the twin of system/sim_param.txt,
# and the SAME system whose PSCAD trajectory is committed in
# upstream/Test23_theta.csv. Full D'Arco droop grid-forming converter with
# active damping ON (kad = 0.2) and split power-filter corners
# (ωz = 2π·5, ωf = 1000), on the OMIB against the ideal source; reference
# power step 0.5 -> 0.7 at t = 1 s. Regenerate with:
#
#   julia --project=hermess/tests/references/psid/_generate \
#         hermess/tests/references/pscad/test23/generate.jl
...
run_and_write(
    case_dir,
    "Droop grid-forming converter (full D'Arco set, kad = 0.2, split power " *
    "filters) on the OMIB, P_ref step 0.5 -> 0.7 at t = 1 s; the system of " *
    "the PSCAD Test23 benchmark.",
    sys,
    ...
    reltol = 1e-6,
    notes = [
        "hermess: GridForming with inner = CascadedDamped (Kad, omega_ad) " *
        "and omega_f_q = 1000; state map as in the psid/gfm_droop case plus " *
        "phi_d/phi_q.",
        "The PSCAD trajectory upstream/Test23_theta.csv starts at t = 9 s " *
        "(offset 9.0, subtract before comparing) and holds θ_oc in rad.",
    ],
)
