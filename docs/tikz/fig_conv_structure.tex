...
\begin{tikzpicture}[node distance=8mm and 12mm]
  ...
  % the PLL sits directly under the network block, so its input drops in
  % vertically without touching any other block
  \node[block, minimum width=18mm] (pll) at (angle -| grid) {PLL\\[-1pt]{\footnotesize\texttt{pll =}}};
  \draw[sig] (meas) -- (angle) node[midway, right, lbl] {$\tilde p_c$};
  % reactive power to the voltage controller, routed through the free lane
  % between the two block columns
  \draw[sig] ($(meas.west)+(0,-0.15)$) -- ++(-0.5,0) |- ($(volt.east)+(0,0.25)$)
    node[pos=0.25, left, lbl] {$\tilde q_c$};
  ...
  \node[lbl, text=ethgrey, align=center] at ($(angle.south)+(0,-0.75)$)
    {\footnotesize \textbf{GridForming}: droop angle source;\ \ \textbf{GridSupporting}: PLL angle source; each block is a pluggable\\[-1pt]
     \footnotesize strategy (\textbf{GridFollowing} is a separate current-injecting chain)};
\end{tikzpicture}
\end{document}
